...
// Export the existing streamQuestion function for backward compatibility
export async function streamQuestion(
  ...
  try {
    await streamConversation(question, (eventType, data) => {
      if (eventType === 'ai' && data.text) {
        onAiUpdate(data.text);
      } else if (eventType === 'result') {
        onResultUpdate(data);
      } else if (eventType === 'data' && data.content && typeof data.content === 'object') {
        // Handle data event with content object
        if (data.content.type === 'ai' && data.content.text) {
          onAiUpdate(data.content.text);
        } else if (data.content.type === 'result') {
          onResultUpdate(data.content);
        }
      }
    });
  } catch (error) {
